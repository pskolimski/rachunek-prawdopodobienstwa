...
function populateYearFilter() {
  const years = [
    ...new Set(tasksData.map((task) => task.year).filter(Boolean)),
  ].sort((a, b) => a - b);
  years.forEach((year) => {
...
function filterTasks() {
  ...
  return tasksData
    .filter((task) => {
      if (state.year !== "all" && String(task.year) !== state.year)
        return false;
...
function renderStats(filtered) {
  ...
  const withSolutions = tasksData.filter(
    (task) => task.status === "complete"
  ).length;
  const flagged = total - withSolutions;
  const visibleFlagged = filtered.filter(
    (task) => task.status !== "complete"
  ).length;
...
function createStatCard(value, label, highlightedValue) {
  const extra =
    highlightedValue === undefined
      ? ""
      : `<span class="extra">(w widoku: ${highlightedValue})</span>`;
  return `
...
function renderList(tasks) {
  ...
  if (tasks.length === 0) {
    tasksList.innerHTML =
      "<p>Brak wyników dla podanych filtrów. Spróbuj zmienić kryteria.</p>";
    return;
  }
  ...
  tasks.forEach((task) => {
    ...
    instance.querySelector(".task-source").textContent = `${task.source} • ${
      task.year ?? "brak roku"
    }`;
    ...
    instance.querySelector(
      ".task-answer"
    ).textContent = `Odpowiedź: ${task.answer}`;
    instance.querySelector(".task-solution").innerHTML = formatRichText(
      task.solution,
      state.search
    );
...
function getProgressLabel(value) {
  return (
    PROGRESS_OPTIONS.find((option) => option.value === value)?.label ?? value
  );
}
